...
white_sounds = [
    mixer.Sound(f"assets\\notes\\{white_notes[i]}.wav") for i in range(len(white_notes))
]

black_sounds = [
    mixer.Sound(f"assets\\notes\\{black_notes[i]}.wav") for i in range(len(black_notes))
]
...
def draw_piano(whites, blacks):
    ...
    for i in range(36):
        ...
        for q in range(len(blacks)):
            if blacks[q][0] == i and blacks[q][1] > 0:
                pygame.draw.rect(
                    screen,
                    "green",
                    [23 + (i * 35) + (skip_count * 35), HEIGHT - 300, 24, 200],
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RIGHT and right_oct < 8:
                right_oct += 1
            if event.key == pygame.K_LEFT and right_oct > 0:
                right_oct -= 1
            if event.key == pygame.K_UP and left_oct < 8:
                left_oct += 1
            if event.key == pygame.K_DOWN and left_oct > 0:
                left_oct -= 1
    pygame.display.flip()
pygame.quit()
